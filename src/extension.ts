...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Register commands
    const toggleCommand = vscode.commands.registerCommand('gitBlameInline.toggle', () => {
        blameProvider.toggle();
    });

    const enableCommand = vscode.commands.registerCommand('gitBlameInline.enable', () => {
        blameProvider.enable();
    });

    const disableCommand = vscode.commands.registerCommand('gitBlameInline.disable', () => {
        blameProvider.disable();
    });

    context.subscriptions.push(toggleCommand, enableCommand, disableCommand, blameProvider);
}
...
class GitBlameProvider implements vscode.Disposable {
    ...
    private onDocumentSaved(document: vscode.TextDocument) {
        // Clear cache and update on save
        this.blameCache.delete(document.fileName);
        this.updateBlameAnnotations();
    }

    private onSelectionChanged() {
        // Optional: Update only visible lines for performance
    ...
    private async updateBlameAnnotations() {
        ...
        try {
            ...
            // Silently handle non-git files or other errors
            editor.setDecorations(this.decorationType, []);
        }
    }

    private async getGitBlameData(filePath: string): Promise<Map<number, BlameInfo>> {
        // Check cache first
    ...
    private parseGitBlameOutput(output: string): Map<number, BlameInfo> {
        ...
        for (const line of lines) {
            if (line.match(/^[a-f0-9]{40}\s+\d+\s+\d+/)) {
                ...
                // This is the actual code line, create blame info
                const commitData = commits.get(currentCommit);
    ...
    private createDecorations(editor: vscode.TextEditor, blameData: Map<number, BlameInfo>): vscode.DecorationOptions[] {
        ...
        for (let lineNum = 1; lineNum <= document.lineCount; lineNum++) {
            ...
            const range = new vscode.Range(lineNum - 1, line.range.end.character, lineNum - 1, line.range.end.character);
